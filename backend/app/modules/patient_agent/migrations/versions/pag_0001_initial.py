...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "session_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("patient_agent_sessions.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_table(
        ...
        sa.Column(
            "session_id",
            postgresql.UUID(as_uuid=True),
            sa.ForeignKey("patient_agent_sessions.id", ondelete="SET NULL"),
            nullable=True,
        ),
        sa.Column("clinic_id", postgresql.UUID(as_uuid=True), nullable=False),
        sa.Column("patient_id", postgresql.UUID(as_uuid=True), nullable=True),
    ...
    op.create_index(
        "ix_patient_agent_audit_events_session_id", "patient_agent_audit_events", ["session_id"]
    )
    op.create_index(
        "ix_patient_agent_audit_events_clinic_id", "patient_agent_audit_events", ["clinic_id"]
    )
    op.create_index(
        "ix_patient_agent_audit_events_patient_id", "patient_agent_audit_events", ["patient_id"]
    )
    op.create_index(
        "ix_patient_agent_audit_events_event_type", "patient_agent_audit_events", ["event_type"]
    )
    op.create_index(
        "ix_patient_agent_audit_events_created_at", "patient_agent_audit_events", ["created_at"]
    )
