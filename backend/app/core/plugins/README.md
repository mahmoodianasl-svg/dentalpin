...
| `loader.py` | Discovery: entry points (PyPI) + filesystem scan (dev). Builds an inventory catalog, then mounts routers/subscribes events/registers tools only for the persisted `installed` allowlist. |
| `registry.py` | `module_registry` singleton — separate discovered-code catalog and active-runtime map. Permissions, tools, events, jobs, and tenancy consume the active map only. |
...
3. `loader.py` — how modules are discovered, reconciled, and selectively activated at boot.
