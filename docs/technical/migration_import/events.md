...
require a committed target row, currently `patient.created` and
`appointment.scheduled`, are queued per mapper context and released after the
containing batch commits; a failed entity savepoint discards its queued
events.
